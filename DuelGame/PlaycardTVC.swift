...
class PlaycardTVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PlaycardshowCell", for: indexPath) as! PlaycardshowCell
        let carddetail = user.playcards[indexPath.row]
        cell.ivImage.image = UIImage(named: "\(carddetail)")
        cell.lbcardname.text = card(cardnumber: carddetail).Name
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let delete = UITableViewRowAction(style: .destructive, title: "Delete", handler: { (action, indexPath) in
            for owncard in user.owncards{
                if owncard.carno == user.playcards[indexPath.row]{
                    owncard.count += 1
                }
            }
            user.playcards.remove(at: indexPath.row)
            self.playcard()
